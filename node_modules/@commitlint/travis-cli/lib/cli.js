...
var _cli = _interopRequireDefault(require("@commitlint/cli"));
...
function main() {
  return new Promise(function ($return, $error) {
    let pop, args;
    validate(); // Stash changes in working copy if needed

    return Promise.resolve(stash()).then(function ($await_2) {
      try {
        pop = $await_2;
        return Promise.resolve(Promise.all([() => fetch({
          name: 'base',
          url: `https://github.com/${REPO_SLUG}.git`
        }), IS_PR ? () => fetch({
          name: 'source',
          url: `https://github.com/${PR_SLUG}.git`
          ...
          try {
            return Promise.resolve(pop()).then(function ($await_4) {
              try {
                ...
                if (IS_PR && RANGE) {
                  let _RANGE$split$filter, _RANGE$split$filter2, start, end;

                  _RANGE$split$filter = RANGE.split('.').filter(Boolean), _RANGE$split$filter2 = (0, _slicedToArray2.default)(_RANGE$split$filter, 2), start = _RANGE$split$filter2[0], end = _RANGE$split$filter2[1];
                  return Promise.resolve(lint(['--from', start, '--to', end, ...args])).then(function ($await_5) {
                    try {
                  ...
                  return Promise.resolve(log(COMMIT)).then(function ($await_6) {
                    try {
                      input = $await_6;
                      return Promise.resolve(lint(args, {
                        input
...
function git(args, options) {
  return new Promise(function ($return, $error) {
    return $return((0, _execa.default)(GIT, args, Object.assign({}, {
      stdio: 'inherit'
    }, options)));
  });
}
...
function lint(args, options) {
  return new Promise(function ($return, $error) {
    return $return((0, _execa.default)(COMMITLINT || _cli.default, args, Object.assign({}, {
...
function log(hash) {
  return new Promise(function ($return, $error) {
    let result;
    return Promise.resolve((0, _execa.default)(GIT, ['log', '-n', '1', '--pretty=format:%B', hash])).then(function ($await_11) {
      try {
        result = $await_11;
